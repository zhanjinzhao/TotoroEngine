...
TBinaryReader::TBinaryReader(_In_z_ wchar_t const* fileName) noexcept(false) :
	mPos(nullptr),
	mEnd(nullptr)
{
	size_t dataSize;
	...
	if (FAILED(hr))
	{
		DebugTrace("ERROR: TBinaryReader failed (%08X) to load '%ls'\n",
			static_cast<unsigned int>(hr), fileName);
		throw std::runtime_error("TBinaryReader");
	}
...
TBinaryReader::TBinaryReader(_In_reads_bytes_(dataSize) uint8_t const* dataBlob, size_t dataSize) noexcept :
	mPos(dataBlob),
...
HRESULT TBinaryReader::ReadEntireFile(
